...
(function ($) {
  ...
  $.widget('lj.repostbutton', $.lj.basicWidget, {
    ...
    _loadRepostedList: function _loadRepostedList(ev) {
      var repost = this;

      if (ev) {
        ev.preventDefault();
      }
      if (this._popupLocked) {
        return;
      }
    ...
    _render: function _render(users, nomore, count) {
      var repost = this;
    ...
    _onUpdateButton: function _onUpdateButton(ev) {
      if (!this.locked()) {
        this.toggleRepost();
      }
      ev.preventDefault();
    ...
    _handleAnswer: function _handleAnswer(answer) {
    ...
    _updateButton: function _updateButton(reposted) {
      this._reposted = reposted;
      this.element.toggleClass(this._cl('active'), this._reposted);
    },

    _updateCounter: function _updateCounter(count) {
    ...
    _canRepost: function _canRepost() {
      if (LJ.get('preview')) {
        return false;
      }
      if (!this._remote) {
      ...
      if (this._remote === this._href.journal) {
        return false;
      }
    ...
    toggleRepost: function toggleRepost() {
      var repost = this,
          args = {
      ...
      if (this._reposted) {
        LJ.Api.call('repost.delete', args, function (answer) {
          ...
          if (!answer.error && !answer.reposted && repost._journal === repost._remote && !location.href.match(/\/(friends|times|feed)\b/)) {
            location.reload();
          } else {
            repost._handleAnswer(answer);
          }
